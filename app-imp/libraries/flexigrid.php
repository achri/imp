...
class Flexigrid
{
	...
	*/	
	...
	public function row_number() {
		$no = (($this->CI->input->post('page')-1) * $this->CI->input->post('rp')) + 1;
		return $no;
	}
	 ...
	 * add@param	before string search by ahrie
	 * add@param	after string search by ahrie
	 * @return	void
	 */
	public function validate_post($d_sortname,$d_sortorder,$valid_fields = NULL,$before = FALSE,$after = TRUE) 
	{
		...
		$this->CI->input->post('qtype') != FALSE && $this->CI->input->post('qtype') != ""
			...
			if (is_array($valid_fields))
				if (in_array($this->CI->input->post('qtype'),$valid_fields))
					$this->post_info['swhere'] = $this->searchstr_validator($this->CI->input->post('query'),$this->CI->input->post('qtype'),$before,$after);
				else
	 ...
	 */
	private function searchstr_validator($searchstr,$searchby,$before=true,$after=true)
	{
		$strBefore = '';
		if ($before)
			$strBefore = '%';
		...
		else
		{
			if (trim($searchstr) != "" && $searchby != "")
			{
				...
				foreach ($searchstr_split as $key => $value) 
				{
					if (trim($value) != "")
						if ($key == 0)
							$searchstr_final .= $searchby.' LIKE "'.$strBefore.$value.$strAfter.'%"';
						else
							$searchstr_final .= ' OR '.$searchby.' LIKE "%'.$strBefore.$value.$strAfter.'%"';
				}

...
function kalkulasi(name,value) {
	var jumlah = parseFloat(($.fn.autoNumeric.Strip('jumlah')).replace(/,/g,'')),
		...
		stok_akhir = parseFloat(($.fn.autoNumeric.Strip('inv_akhir')).replace(/,/g,'')),
	...
	switch (name) {
		case 'satuan_id' :
			if (jumlah * jumlah_multi > stok_akhir){
				jumlah = 0;//stok_akhir;
				$('#'+name).val(value);
				$('#jumlah').val(jumlah);
				informasi("Quantity Multi Unit must less or equal than stock in inventory !!!");
			}
		break;
		case 'jumlah' :
			if (value * jumlah_multi > stok_akhir){
				jumlah = 0;//stok_akhir;
				$('#'+name).val(jumlah);
				informasi("Quantity Selling must less or equal than stock in inventory !!!");
			}
		break;
		case 'harga' :
			if (value < last_buying){
				harga = last_buying;
				$('#'+name).val(harga);
				informasi("Price selling must equal or grater than last buying !!!");
			}
		break;
	}
...
$(document).ready(function() {	
	...
	try {
		var $jual_form = $('form#jual_form'),
			$KEY = new Array(9,13,16,17,18,20,33,34,35,36,37,38,39,40), // 8 backspace,46 del
			$SET = {'produk_kode': true, 'produk_nama': true}; // FIRST CONDITION FOR AUTOCOMPLETE SELECTOR
		
		// METHOD KEYUP FOR CLASS AUTOCOMPLETE
		$('input.auto',$jual_form).keyup(function(event){	
			var $item = $(this),
			...
			try {
				if ($check_key == -1) {
					$('input.cl:not(#'+$item_id+')',$jual_form).val('');
			...
			if ($SET[$type]){	
				...
				.result(function(event,item) {
					...
					$('#last_selling').val(item[8]);
					$('#inv_akhir').val(item[9]);
					$('#harga').val(item[7]);
...
$('input:not(#kat_nama,#produk_nama)').attr('size',15);
$('.number').attr('digit_decimal',2);
</script>
<form id="jual_form"> 
<fieldset class="ui-widget-content ui-corner-all"> 
	<legend class="ui-corner-all ui-state-active">SELLING FORM</legend> 
	<table width="100%" class="ui-widget-content" border=0 style="border:0px"> 
		<tbody valign="top"> 
			<tr> 
				...
				<td width="5px">:</td>
				<td width="40%">
					<input class="jual_form" type="hidden" name="jual_id" id="jual_id" title=""/>	
					<input class="jual_form required cl" type="hidden" name="produk_id" id="produk_id" title="Product"/>	
					<input class="jual_form auto cl" type="text" name="produk_kode" id="produk_kode"/>	
				</td>
				<td width="20px">&nbsp;</td>
				<td width="100px">Unit</td> 
				<td width="5px">:</td>
				<td>
					<select class="jual_form required select cl" name="satuan_id" id="satuan_id" title="Unit"/>
					</select>
				</td>
			</tr> 
			<tr> 
				...
				<td>:</td>
				<td>
					<input class="jual_form auto cl" type="text" name="produk_nama" id="produk_nama" size="30"/>
				</td>
				<td>&nbsp;</td>
				<td>Qty</td> 
				<td>:</td>
				<td>
					<input class="jual_form required number kalkulasi" type="text" name="jumlah" id="jumlah" title="Qty"/>
					<input class="beli_form" type="hidden" name="jumlah_multi" id="jumlah_multi" title="Qty Multi Level"/>
				</td>
			</tr> 
			<tr> 
				<td>Category</td> 
				<td>:</td>
				<td>
					<input class="jual_form cl" type="hidden" name="kat_id" id="kat_id"/>
					<input readonly class="jual_form cl" type="text" name="kat_nama" id="kat_nama" size="50"/>
				</td>
				<td>&nbsp;</td>
				<td>Last Buying (Rp)</td>
				<td>:</td>
				<td>
					<input readonly class="jual_form number cl" type="text" name="last_buying" id="last_buying"/>
				</td>
			</tr> 
			<tr>
				<td>Discount</td>
				<td>:</td>
				<td>
					<input class="jual_form number kalkulasi" type="text" name="diskon" id="diskon" title="Discount"/> = 
					<input readonly class="jual_form number kalkulasi" type="text" name="tot_diskon" id="tot_diskon" title="Total Discount"/>
				</td>
				<td>&nbsp;</td>
				<td>Last Selling (Rp)</td>
				<td>:</td>
				<td>
					<input readonly class="jual_form number cl" type="text" name="last_selling" id="last_selling"/>
				</td>
			</tr>
			<tr>
				<td colspan=4>&nbsp;</td>
				...
				<td>:</td>
				<td>
					<input class="jual_form required number kalkulasi" type="text" name="harga" id="harga" title="Price"/>
				</td>
			</tr>
			<tr><td colspan=7><hr></td></tr>
			<tr> 
				<td>
					<input onclick="jual_add(this.value);" type="button" id="jual_tambah" value="ADD" />
				</td>
				<td></td>
				<td>
					<input onclick="jual_edit(this.value);" type="button" id="jual_ubah" value="EDIT" />
					<input onclick="jual_delete(this.value);" type="button" id="jual_hapus" value="DELETE" />
					
					<input onclick="jual_kondisi_awal();" type="button" id="jual_batal" value="CANCEL" />
...
<input class="number" type="hidden" id="inv_akhir" name="inv_akhir">

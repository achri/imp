...
class Jqgrid_model extends CI_Model {
	...
	/*
		...
		@return :	array
	*/
	function get_data_query($SQL=FALSE,$countField=FALSE,$whereFirst=TRUE,$additional=FALSE,$userdata=FALSE,$table=FALSE,$joinmodel=FALSE,$fields=FALSE,$prefix=FALSE) {
		// DEBUGING
            ...
            if(is_array($joinmodel)):            
                foreach($joinmodel as $model):
                    if(isset($model['table']) && isset($model['join'])):
					   ...
					   $arrJoin[] = (isset($model['type']))?$model['type']:'INNER' . " JOIN " . $model['table'] . " ON " . $model['join'];
		...
		if ($filter && $filtertext)
			$SQL .= $a . $tables . '.' . $filter . $arr_oper[$oper] . $filtertext;
		...
		if ($fsearch):
			if ($filters): // FILTER TOOLBAR
				...
				foreach ($fdecode->rules as $fdata):
					$SQL .= $fdecode->groupOp . ' ' . $tables . '.' .$fdata->field . $arr_oper[$fdata->op] . '"' . $fdata->data . '%"';
				endforeach;
			else: // FILTER GRID
				...
				foreach ($get_field->list_fields() as $field):
					...
					if (false == empty($value)):
						$SQL .= ' AND ' . $tables . '.' .$field ." LIKE '" . $value . "%'" ;
					endif;
				endforeach;
			endif;
		endif;
		...
		if( empty($sortdir) )
			$sortdir = "ASC";
		if( !empty($sortby) ):
			$SQL .= " ORDER BY " . $tables . '.' . $sortby . ' ' . $sortdir ;
		endif;
